...
interface VideoUploaderProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   * @default undefined
   * @example progresses={{ "video1.mp4": 50 }}
   */
              ...
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                ...
                <div className="space-y-px">
                  ...
                  <p className="text-sm text-muted-foreground/70">
                    You can upload
                    {maxFiles > 1
                      ? ` up to ${maxFiles} videos (up to ${formatBytes(
                          maxSize
                        )} each)`
                      : ` a video up to ${formatBytes(maxSize)}`}
                  </p>
                </div>
              </div>
            )}
...
function VideoCard({ file, progress, onRemove }: VideoCardProps) {
  return (
